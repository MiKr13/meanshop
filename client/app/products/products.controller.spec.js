...
describe('Controller: ProductsCtrl', function () {
  ...
  beforeEach(inject(function ($rootScope, _Products_/*, $state*/) {
    scope = $rootScope.$new();
    mockProduct = validAttributes[0];
    Products = _Products_;

    sinon.stub(Products, 'query').returns(validAttributes);
    sinon.stub(Products, 'get').returns(mockProduct);
  ...
  describe('ProductViewCtrl', function() {
    ...
    it('should remove product and redirect if succeded', function() {
      var stub = sinon.stub(Products, 'delete', callCallbackWithError());
      scope.deleteProduct();
    ...
    it('should not redirect if an error occurs', function() {
      sinon.stub(Products, 'delete', callCallbackWithError(true));
      scope.deleteProduct();
  ...
  describe('ProductNewCtrl', function() {
    ...
    it('should create a new product and redirect to products', function() {
      var stub = sinon.stub(Products, 'save', callCallbackWithError(false));
    ...
    it('should not redirect if save fails', function() {
      sinon.stub(Products, 'save', callCallbackWithError(true));
      scope.product = mockProduct;
      scope.addProduct();
      expect(state.go.calledOnce).to.equal(false);
    });
  ...
  describe('ProductEditCtrl', function() {
    ...
    it('should edit product and redirect to view if success', function() {
      var stub = sinon.stub(Products, 'update', callCallbackWithError(false));
      scope.editProduct();
      assert(stub.withArgs({id: mockProduct._id}).calledOnce);
    ...
    it('should not redirect if failed', function() {
      sinon.stub(Products, 'update', callCallbackWithError(true, mockProduct));
      scope.editProduct();
      expect(state.go.calledOnce).to.equal(false);
    });
  });
